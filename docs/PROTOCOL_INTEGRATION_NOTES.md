...
For the detailed official-derived reference, see [DEEPBOOK_PREDICT_OFFICIAL_CONTRACT_INFO.md](./DEEPBOOK_PREDICT_OFFICIAL_CONTRACT_INFO.md). For Phase 1B wallet and Predict Account flow details, see [PREDICT_MANAGER_FLOW.md](./PREDICT_MANAGER_FLOW.md). For the Phase 1B-Verify local signer validation report, see [PREDICT_MANAGER_TESTNET_VALIDATION.md](./PREDICT_MANAGER_TESTNET_VALIDATION.md). For the Phase 1C range quote and mint validation report, see [RANGE_MINT_TESTNET_VALIDATION.md](./RANGE_MINT_TESTNET_VALIDATION.md). For the Phase 1C-fix quoteability scanner investigation, see [RANGE_QUOTEABILITY_INVESTIGATION.md](./RANGE_QUOTEABILITY_INVESTIGATION.md). For the Phase 1C-fix2 quantity, decoding, and binary quote investigation, see [RANGE_QUOTE_UNITS_AND_DECODING.md](./RANGE_QUOTE_UNITS_AND_DECODING.md). For SDK/PTB entrypoint tracking, see [ENTRYPOINT_BINDINGS_PLAN.md](./ENTRYPOINT_BINDINGS_PLAN.md).
...
| Range minting | Phase 1C-fix2 found positive official range quotes and reached the real mint path. | Positive quote found / mint blocked by ask gate | The previous `pricing_config::quote_spread_from_fair_price` abort code `1` means invalid boundary fair price. Phase 1C-fix2 range sweep found `quantity=1 mint_cost=1`, binary quote sanity found nonzero `get_trade_amounts` output, and `validate:range-mint` failed before digest with `predict::assert_mintable_ask` abort code `7`. See [RANGE_MINT_TESTNET_VALIDATION.md](./RANGE_MINT_TESTNET_VALIDATION.md), [RANGE_QUOTEABILITY_INVESTIGATION.md](./RANGE_QUOTEABILITY_INVESTIGATION.md), and [RANGE_QUOTE_UNITS_AND_DECODING.md](./RANGE_QUOTE_UNITS_AND_DECODING.md). |
...
| Ask bounds | Selected oracles returned `null` | Diagnostic / MUST CONFIRM BEFORE CODING for final UX | Public server `/oracles/:oracle_id/ask-bounds`, direct object reads, or generated bindings | Phase 1C-fix does not treat `null` ask bounds as the primary blocker; quote/mint gating relies on official quote output and safety gates. |
...
| First real `mint_range<DUSDC>` transaction validation | Positive quote found; mint attempt failed in `predict::assert_mintable_ask` with abort code `7` before digest | MUST CONFIRM BEFORE CODING | Pinned source branch, mint ask gate, Testnet transaction execution, and post-transaction readback | Gated validation selected a positive quote and manager balance coverage, but official mint ask gate rejected the transaction. Confirm abort meaning before retrying mint. |
...
| 6 | `predict::mint_range<DUSDC>` | Mint the guided range prediction | Real Testnet mint path reached, but failed before digest in `predict::assert_mintable_ask` with abort code `7` |
