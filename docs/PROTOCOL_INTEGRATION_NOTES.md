...
For DeepVol direction and binary-leg requirements, see [DEEPVOL_PRODUCT_DIRECTION.md](./DEEPVOL_PRODUCT_DIRECTION.md), [DEEPVOL_PRIMITIVES_AND_RECEIPTS.md](./DEEPVOL_PRIMITIVES_AND_RECEIPTS.md), [DEEPVOL_MVP_SCOPE.md](./DEEPVOL_MVP_SCOPE.md), [DEEPVOL_PROTOCOL_ARCHITECTURE.md](./DEEPVOL_PROTOCOL_ARCHITECTURE.md), [DEEPVOL_BINARY_LEG_INTEGRATION.md](./DEEPVOL_BINARY_LEG_INTEGRATION.md), [DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md](./DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md), and [ADR/0003-pivot-to-deepvol-btc-move.md](./ADR/0003-pivot-to-deepvol-btc-move.md).
...
Binary quote/read/preflight has diagnostic evidence, but full two-leg binary mint remains blocked before submission and binary redeem remains unvalidated. The 2026-05-19 controlled binary mint round passed read, balance, owner, CLI environment, transaction-shape, and `devInspect` gates, then stopped at CLI dry-run with `InsufficientGas in command 3`; no write transaction was submitted and no retry was attempted. Active BTC oracle, expiry, strike grid, and leg mintability are `MUST CONFIRM AT RUNTIME`. Quote success and `devInspect` success do not imply executable mint submission; future mints still require fresh gates and dry-run/wallet simulation before approval. Range and wrapper validations remain useful prior work: they proved PredictManager setup, full preflight discipline, direct manager readback, ProtocolVault fee custody, and wrapper/protocol event linkage.
...
| DeepVol two-leg binary mint | Controlled round blocked before submission: CLI dry-run returned `InsufficientGas in command 3` | Blocked / MUST CONFIRM BEFORE CODING | Full PTB devInspect, CLI dry-run diagnosis, and later controlled Testnet validation | 2026-05-19 round selected oracle `0xc746336e790db7e93a34b684fa3768f43a7c3171d0262d0f2c71dc0a2ab5fe22`, expiry `1779436800000`, strikes `76221000000000 / 76334000000000`, quantity `1000`, total quote `1003`; no write transaction submitted. |
| DeepVol binary readback | Direct `predict_manager::position` devInspect returned UP `0` and DOWN `0` before the blocked mint round | Partial / MUST CONFIRM BEFORE CODING | Direct devInspect/readback helper | Required for wallet-critical UP and DOWN quantities because non-custodial `MoveReceipt` does not own legs. |
...
| 11 | `predict::mint<DUSDC>` | Mint binary UP/DOWN legs for DeepVol BTC MOVE | Two-leg PTB `devInspect` verified; 2026-05-19 controlled CLI dry-run blocked with `InsufficientGas in command 3` before submission |
...
| 13 | `predict_manager::position` | Read binary position quantity by `MarketKey` | Direct devInspect readback exercised by the DeepVol harness for selected UP/DOWN keys |
