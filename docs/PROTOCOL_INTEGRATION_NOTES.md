...
For DeepVol direction and binary-leg requirements, see [DEEPVOL_PRODUCT_DIRECTION.md](./DEEPVOL_PRODUCT_DIRECTION.md), [DEEPVOL_PRIMITIVES_AND_RECEIPTS.md](./DEEPVOL_PRIMITIVES_AND_RECEIPTS.md), [DEEPVOL_MVP_SCOPE.md](./DEEPVOL_MVP_SCOPE.md), [DEEPVOL_FRONTEND_MVP.md](./DEEPVOL_FRONTEND_MVP.md), [DEEPVOL_PROTOCOL_ARCHITECTURE.md](./DEEPVOL_PROTOCOL_ARCHITECTURE.md), [DEEPVOL_MOVE_RECEIPT_CONTRACT.md](./DEEPVOL_MOVE_RECEIPT_CONTRACT.md), [DEEPVOL_CONTRACT_BUILD_VALIDATION.md](./DEEPVOL_CONTRACT_BUILD_VALIDATION.md), [DEEPVOL_BINARY_LEG_INTEGRATION.md](./DEEPVOL_BINARY_LEG_INTEGRATION.md), [DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md](./DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md), [DEEPVOL_TESTNET_PUBLISH_RESULT.md](./DEEPVOL_TESTNET_PUBLISH_RESULT.md), [DEEPVOL_BUY_MOVE_RECEIPT_TESTNET_VALIDATION.md](./DEEPVOL_BUY_MOVE_RECEIPT_TESTNET_VALIDATION.md), and [ADR/0003-pivot-to-deepvol-btc-move.md](./ADR/0003-pivot-to-deepvol-btc-move.md).
...
Binary quote/read/preflight and one controlled full two-leg binary mint now have Testnet evidence; binary redeem remains unvalidated. The 2026-05-19 controlled binary mint diagnosis reproduced `InsufficientGas in command 3` at `100000000` MIST, identified command `3` as the second `predict::mint` if zero-based and `market_key::down` if one-based, then passed SDK and CLI dry-runs at `200000000` MIST and executed one real two-leg BTC binary mint with digest `4fMQtu8mFB6jLa5gtSWBsDj3gYp8u9AjQw3xs2VcNJoh`. Active BTC oracle, expiry, strike grid, and leg mintability are `MUST CONFIRM AT RUNTIME`. Quote success and `devInspect` success do not imply executable mint submission; future mints still require fresh gates and dry-run/wallet simulation before approval. Range and wrapper validations remain useful prior work: they proved PredictManager setup, full preflight discipline, direct manager readback, ProtocolVault fee custody, and wrapper/protocol event linkage.
...
| 13 | `predict::redeem<DUSDC>` / `predict::redeem_permissionless<DUSDC>` | Redeem binary positions for guided settlement | Source-confirmed; validation pending |
