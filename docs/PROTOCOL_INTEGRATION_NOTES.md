...
Phase 3D replaces direct platform-recipient routing with RangePilot `ProtocolVault<T>` and `AdminCap`. The wrapper transfers creator fees to the creator, deposits platform fees into `ProtocolVault<T>`, returns any fee coin remainder to the follower, and then calls DeepBook Predict `mint_range<T>`. Confirmed policy is `platform_fee_bps = 10` and `MAX_CREATOR_FEE_BPS = 3000`; `3000 bps = 30%` and `300 bps = 3%`. Strategy objects remain shared, strategy creation remains permissionless, metadata policy is `metadata_uri`, and the Testnet/hackathon wrapper is upgradeable with upgrade authority to be disclosed before publish.

Formal wrapper dependency source: official DeepBookV3 Git dependencies for `packages/predict` and `packages/deepbook` at `predict-testnet-4-16`, with Testnet `dep-replacements` binding the deployed DeepBook Predict and DeepBook package IDs. The local third-party snapshot root `deepbookv3-predict-testnet-4-16/` is ignored and must not be committed; it is source-level debugging/reference only, not the formal Move dependency source. Official Sui docs and `DEEPBOOK_PREDICT_OFFICIAL_CONTRACT_INFO.md` remain the deployment/config source of truth. The wrapper package is unpublished, package ID is `TBD`, ProtocolVault object ID is `TBD`, and client builders must block unless explicit wrapper package ID and protocol vault ID are supplied. Phase 3E confirmed the intended publisher/AdminCap owner address as `0xc558e37d20405a9751c81124ac8d167e2b2d368b834319adafa549449e0715f5`, but no actual AdminCap exists because publish was blocked before execution.

Build status: `npm run move:build:rangepilot` passes with official DeepBookV3 Git dependencies and Testnet dep-replacements, and `npm run move:test:rangepilot` passes with 18 RangePilot tests. Phase 3E pre-publish `npm run typecheck` and `npm run build:web` also passed, but `sui client publish move/rangepilot --dry-run --gas-budget 200000000 --json` and `sui move build --path move/rangepilot --build-env testnet --dump-bytecode-as-base64` still classify `deepbook_predict` as an unpublished dependency. The old incomplete `deepbookv3-predict-package/predict` dependency blocker and the temporary local snapshot formal dependency are superseded, and the wrapper must not switch back to local snapshot dependencies.
