...
| Range minting | Phase 1C-fix2 found positive official range quotes and reached the real mint path; Phase 1C-fix3 added full mint preflight. | Positive quote found / no mintable candidate yet | The previous `pricing_config::quote_spread_from_fair_price` abort code `1` means invalid boundary fair price. Phase 1C-fix2 range sweep found `quantity=1 mint_cost=1`, binary quote sanity found nonzero `get_trade_amounts` output, and `validate:range-mint` failed before digest with `predict::assert_mintable_ask` abort code `7`, now identified as `EAskPriceOutOfBounds`. Phase 1C-fix3 scanned `120` quote candidates across `960` quote attempts and ran `40` full mint preflights; `0` preflights passed, `29` failed with code `7`, and `11` failed with other aborts. A positive quote is not sufficient; full `mint_range<DUSDC>` preflight must pass before real mint. See [RANGE_MINT_TESTNET_VALIDATION.md](./RANGE_MINT_TESTNET_VALIDATION.md), [RANGE_QUOTEABILITY_INVESTIGATION.md](./RANGE_QUOTEABILITY_INVESTIGATION.md), [RANGE_QUOTE_UNITS_AND_DECODING.md](./RANGE_QUOTE_UNITS_AND_DECODING.md), and [MINTABILITY_PREFLIGHT_AND_ASK_BOUNDS.md](./MINTABILITY_PREFLIGHT_AND_ASK_BOUNDS.md). |
...
`npm run validate:range-mint` reached the real Testnet mint submission path after quote safety gates passed, but transaction resolution failed before returning a digest with `MoveAbort` code `7` in `predict::assert_mintable_ask`. From pinned source, code `7` is `EAskPriceOutOfBounds`, so quote success must not authorize real mint by itself. Phase 1C-fix3 added onchain `predict::ask_bounds` investigation and full `mint_range<DUSDC>` devInspect preflight. The bounded mintable-range scan derived `390` candidate ranges, tested `120` quote candidates across `960` quote attempts, and found `0` full preflight successes across `40` preflight attempts. `npm run validate:range-quote` selected a positive quote but blocked on full preflight with code `7` / `EAskPriceOutOfBounds`, so no Phase 1C-fix3 real mint was submitted. First successful mint, `RangeMinted` event shape, and post-mint portfolio readback remain pending.
...
| Exact generated-binding/PTB call shapes | `create_manager` and gated `deposit<DUSDC>` PTBs validated on Testnet; range and binary quote PTBs produced successful decoded quote returns | Partial / MUST CONFIRM BEFORE CODING for range writes | Pinned source branch, generated bindings, devInspect, and real Testnet transaction attempts | `range_key::new` plus `get_range_trade_amounts`, and `market_key::up/down` plus `get_trade_amounts`, are devInspect-verified for quote previews; mint write remains unverified. |
| First real `mint_range<DUSDC>` transaction validation | Positive quote found; mint attempt failed in `predict::assert_mintable_ask` with abort code `7` / `EAskPriceOutOfBounds` before digest; Phase 1C-fix3 found `0` full preflight successes in `40` preflight attempts | MUST CONFIRM BEFORE CODING | Pinned source branch, onchain ask bounds, full mint preflight, Testnet transaction execution, and post-transaction readback | Gated validation selected a positive quote and manager balance coverage, but official mint ask gate rejected the transaction. Phase 1C-fix3 keeps real mint blocked until full preflight succeeds. |
...
| 4 | `range_key::new` | Construct the range key for lower/upper strikes | Phase 1C-fix scanner builds spot/forward-centered ranges and successful quote devInspect returns, so the helper is exercised on the success path |
| 5 | `predict::get_range_trade_amounts` | Preview official range trade amounts | Phase 1C-fix2 devInspect verified safe `(mint_cost, redeem_payout)` decoding and found positive official range quotes |
| 6 | `predict::mint_range<DUSDC>` | Mint the guided range prediction | Real Testnet mint path reached, but failed before digest in `predict::assert_mintable_ask` with abort code `7`; full preflight scanner found no mintable candidate yet |
