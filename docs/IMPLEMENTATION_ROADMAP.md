...
| Phase 1C: Range quote + first `mint_range<DUSDC>` | Confirm `range_key::new`, active market, strike grid, ask bounds, `predict::get_range_trade_amounts`, quote preview mapping, full mint preflight, and first `predict::mint_range<DUSDC>` transaction after Phase 1B blockers clear. | Phase 1C-fix2 verified safe `get_range_trade_amounts` decoding, found positive official range quotes, and confirmed binary `get_trade_amounts` can return nonzero quotes. The first real mint attempt reached the Testnet write path but failed before digest with `predict::assert_mintable_ask` abort code `7` / `EAskPriceOutOfBounds`; Phase 1C-fix3 validated onchain ask-bounds, added full mint preflight, and found no mintable candidate across `40` preflight attempts, so real mint remains blocked. |
