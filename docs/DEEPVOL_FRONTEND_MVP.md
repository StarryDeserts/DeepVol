...
Status: DeepVol-25 adds wallet-scoped PredictManager session UX and primitive Portfolio readback. DeepVol-24-fix adds RANGE mintability diagnostics while preserving the RANGE wallet gate. DeepVol-23: UP/DOWN primitive direct mint validated on Testnet; RANGE execution path added (pending validation); see DEEPVOL_PRIMITIVE_UP_DOWN_VALIDATION.md and DEEPVOL_RANGE_PRIMITIVE_TRADING.md. DeepVol-21 adds mintable strike candidate search for UP/DOWN primitives; execution gate now requires primitiveMintabilityStatus passed. Updated for DeepVol-20: fresh BTC MOVE buy validated on Testnet with active VolSeries. BuyMovePage detects stale/missing/validation-required/non-mintable VolSeries, provides Regenerate mintable range, and gates Create BTC MOVE Series plus buy behind mintability validation. See DEEPVOL_ACTIVE_MOVE_SERIES.md, DEEPVOL_MINTABLE_MOVE_RANGE.md, DEEPVOL_PREDICT_MANAGER_UX.md, and DEEPVOL_PORTFOLIO_PRIMITIVE_POSITIONS.md.
...
| `/buy/btc-move` | Transaction workspace with product context, first-time flow checklist, wallet-scoped PredictManager session, Create PredictManager CTA, DUSDC wallet/deposit visibility, quantity setup, quote metrics, UP/DOWN leg quotes, explicit preflight, disabled buy CTA, and advanced protocol details. |
| `/primitives` | Primitive workspace for UP / DOWN / RANGE active BTC market refresh, `Live / Stale / Expired / Unknown` status, selected oracle/expiry details, shared PredictManager session, Advanced / Developer manager fallback, quote refresh, manager balance, mint preflight diagnostics, UP/DOWN wallet-gated execution, and RANGE wallet-gated execution (pending Testnet validation). |
| `/portfolio` | Receipt overview showing local browser records or validation reference artifacts, explicit local/indexer limitations, DeepVol-10 browser receipt display validation, DeepVol-13 browser guided redeem validation, separate MOVE Receipts and Primitive Positions sections, and per-record known-key primitive readback. |
